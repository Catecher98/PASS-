...
class protoAugSSL:
    def __init__(self, args, file_name, feature_extractor, task_size, device):
        ...
        self.model = network(args.fg_nc*4, feature_extractor) # fg_nc == number of first step
        self.radius = 0
        self.prototype = None
        self.class_label = None
        self.numclass = args.fg_nc # 50
        ...
        self.layer = 'layer4'
        # print(self.model.feature.layer4)
        print(self.model)
    ...
    def train(self, current_task, old_class=0):
        ...
        accuracy = 0
        print("============================ start training the {}th phase ...=========================".format(current_task))
        for epoch in range(self.epochs):
            ...
            for step, (indexs, images, target) in enumerate(self.train_loader):
                ...
                opt.zero_grad()
                loss = self._compute_loss(images, target.long(), old_class) # output
                opt.zero_grad()
                loss.backward()
                opt.step()
    ...
    def _compute_loss(self, imgs, target, old_class=0):
        ...
        if self.old_model is None:
            ...
            # ============================== Grad Knowledge Distilltaion ======================
            # =================================================================================
            with GradCAM(self.old_model, self.layer) as gradcam_old:
                attmap_old, outputs_old = gradcam_old(imgs, return_outputs=True)
            with GradCAM(self.model, self.layer) as gradcam:
                attmap = gradcam(imgs)
            attention_map1 = torch.norm(attmap_old, p=2, dim=1)
            attention_map2 = torch.norm(attmap, p=2, dim=1)
            loss_AD = torch.norm(attention_map2 - attention_map1, p=1, dim=1).sum(dim=1).mean()
            # ==================================================================================
            # ============================  end  ===============================================
            feature, feature_mid = self.model.feature(imgs)
            feature_old, feature_mid_old = self.old_model.feature(imgs)
            ...
            loss_MLD = loss_kd + loss_AD
            # print("loss_cls:{}, loss_protoAug:{}, loss_kd:{}, loss_AD:{}".format(loss_cls, loss_protoAug, loss_kd, loss_AD))
            return loss_cls + self.args.protoAug_weight * loss_protoAug + self.args.kd_weight * loss_MLD
    ...
    def protoSave(self, model, loader, current_task):
        ...
        with torch.no_grad():
            for i, (indexs, images, target) in enumerate(loader):
                feature, feature_mid = model.feature(images.to(self.device))
        ...
        if current_task == 0:
            ...
            print(self.radius)
            print("shape of prototype: ", len(self.prototype))
            print(len(self.prototype))
        else:
            ...
            print("after concate:", len(self.prototype))
            print("after concate y :", len(self.class_label))
